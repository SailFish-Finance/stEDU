...
describe("stEDU and wstEDU Integration", function () {
  ...
  describe("Basic Integration", function () {
    it("Should handle stake → wrap → unwrap → unstake cycle", async function () {
      ...
      expect(stEDUBalance).to.equal(
        calculateStEDUAmount(stakeAmount, INITIAL_INDEX)
      );
      ...
      expect(wstEDUBalance).to.equal(
        calculateWstEDUAmount(stEDUBalance, INITIAL_INDEX)
      );
      ...
      expect(finalEDUBalance + totalGasCost).to.be.closeTo(
        initialEDUBalance,
    ...
    it("Should maintain correct value relationship after rewards", async function () {
      ...
      const initialStEDUValue = calculateEDUAmount(
        initialStEDUBalance,
        initialIndex
      );
      ...
      expect(await wstEDU.balanceOf(user.address)).to.equal(
        initialWstEDUBalance
      );
      ...
      const wstEDUIncrease =
        (newWstEDUValue * BigInt(1e18)) / initialWstEDUValue;
    ...
    it("Should handle wrap → rewards → unwrap correctly", async function () {
      ...
      // Calculate initial EDU value of stEDU
      ...
      // The stEDU amount should be the same as initially wrapped
      const unwrappedStEDUBalance = await stEDU.balanceOf(user.address);
      expect(unwrappedStEDUBalance).to.equal(stEDUBalance);
      
      // But the EDU value should be greater due to rewards
      const newIndex = await stEDU.index();
      const newEDUValue = calculateEDUAmount(unwrappedStEDUBalance, newIndex);
      expect(newEDUValue).to.be.gt(initialEDUValue);
      
      // The increase should be proportional to rewards
      const expectedIncrease = (TEN_EDU * BigInt(1e18)) / await stEDU.totalStaked();
  ...
  describe("Multi-User Integration", function () {
    it("Should handle multiple users staking, wrapping, and rewards correctly", async function () {
      ...
      expect(await wstEDU.balanceOf(user1.address)).to.equal(
        user1WstEDUBalance
      );
      ...
      // User 1's stEDU should be the same as initially wrapped
      const user1UnwrappedStEDU = await stEDU.balanceOf(user1.address);
      expect(user1UnwrappedStEDU).to.equal(user1StEDUBalance);
      
      // But the EDU value should be greater due to rewards
      const newIndex = await stEDU.index();
      const initialEDUValue = calculateEDUAmount(user1StEDUBalance, initialIndex);
      const newEDUValue = calculateEDUAmount(user1UnwrappedStEDU, newIndex);
      expect(newEDUValue).to.be.gt(initialEDUValue);
  ...
  describe("Index Change Tests", function () {
    ...
    it("Should verify wstEDU maintains constant share after rewards", async function () {
      ...
      const initialStEDUEquivalent = calculateStEDUFromWstEDU(
        wstEDUBalance,
        initialIndex
      );
      const initialShare =
        (initialStEDUEquivalent * BigInt(1e18)) / initialTotalStaked;
      ...
      const newStEDUEquivalent = calculateStEDUFromWstEDU(
        wstEDUBalance,
        newIndex
      );
      const newShare = (newStEDUEquivalent * BigInt(1e18)) / newTotalStaked;

      // With our implementation, the share will increase proportionally to the rewards
      // This is expected behavior with the non-rebasing wrapper
      expect(newShare).to.be.gt(initialShare);
      
      // The increase should be proportional to rewards
      const expectedShareIncrease = (TEN_EDU * BigInt(1e18)) / initialTotalStaked;
      const actualShareIncrease = newShare - initialShare;
      expect(actualShareIncrease).to.be.closeTo(expectedShareIncrease, BigInt(1e16));
    });
  });
});
